import {
  ...
  export const createSubDomain = async (data) => {
    try {
      ...
      if (res.data.status == "success") {
        return "Created Dao Account successfully!";
  ...
  export const mintCollection = async (
    ...
    try {
      ...
      // const resData = await connection.getParsedTransaction(
      //     tx,
      ...
      } else if (errMsg.indexOf("Dao Member Name  already exist") > 0) {
        return ["Member Name is taken", "", ""];
      } else {
        return ["error", "", ""];
      }
    }
  ...
  export const mintNFTCollection = async (
    ...
    try {
      ...
      let realURI =
        uri == "uri"
          ? `{your_Texaglo_api}/ipfs?domain=${subdomain}&tokenId=${tokenId}`
          : uri.uri;
    ...
    try {
      ...
      for (let i = 0; i < txs.length; i++) {
        ...
        if (logs.length == 93 || logs.length == 94) {
          if (logs[35].includes("{your_Texaglo_api}")) {
            domains.push(logs[34].split("log:")[1].trim());
          }
        }
        if (logs.length == 95) {
          if (logs[37].includes("{your_Texaglo_api}")) {
